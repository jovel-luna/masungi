...
class Research extends Model
{
    ...
    public static function store($request, $item = null, $columns = ['name', 'description'])
    {
        ...
        if ($request->hasFile('image_path')) {
            $item->storeImage($request->file('image_path'), 'image_path', 'researches-images');
        }
    ...
    public function renderShowUrl($prefix = 'admin') {
        ...
        return route($prefix . '.researches.show', $route);
    }

    public function renderArchiveUrl($prefix = 'admin') {
        return route($prefix . '.researches.archive', $this->id);
    }

    public function renderRestoreUrl($prefix = 'admin') {
        return route($prefix . '.researches.restore', $this->id);
    }

    public function renderRemoveImageUrl($prefix = 'admin') {
        return route($prefix . '.researches.remove-image', $this->id);
    }
}
